"use client";

import { useActionState } from "react";
import Link from "next/link";
import { loginDocente } from "@/lib/actions/auth-actions";
import { MeshGradient } from "@/components/ui/mesh-gradient";

export default function LoginPage() {
  const [state, formAction, pending] = useActionState(loginDocente, undefined);

  return (
    <div className="relative flex min-h-screen items-center justify-center px-4 py-10">
      <MeshGradient />
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-[0_30px_80px_-30px_rgba(30,35,64,0.25)]">
        <div className="mb-6 flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-primary text-[17px] font-extrabold text-white">
            A
          </div>
          <div className="text-[19px] font-extrabold tracking-tight text-text">Aulera</div>
        </div>

        <h1 className="text-2xl font-extrabold text-text">Iniciá sesión</h1>
        <p className="mt-1 text-sm text-text-faint">Volvé a tu espacio de trabajo.</p>

        <form action={formAction} className="mt-6 flex flex-col gap-4">
          <label className="flex flex-col gap-1.5">
            <span className="text-[13px] font-semibold text-text">Email</span>
            <input
              name="email"
              type="email"
              required
              placeholder="marina.garcia@mail.com"
              className="rounded-[11px] border border-border bg-surface px-3.5 py-2.5 text-sm text-text outline-none focus:border-primary"
            />
          </label>
          <label className="flex flex-col gap-1.5">
            <span className="text-[13px] font-semibold text-text">Contraseña</span>
            <input
              name="password"
              type="password"
              required
              className="rounded-[11px] border border-border bg-surface px-3.5 py-2.5 text-sm text-text outline-none focus:border-primary"
            />
          </label>

          {state?.error && (
            <p className="rounded-[10px] bg-danger-soft px-3.5 py-2.5 text-[13px] text-danger">
              {state.error}
            </p>
          )}

          <button
            type="submit"
            disabled={pending}
            className="mt-2 rounded-[11px] bg-primary px-5 py-3 text-sm font-bold text-white transition hover:bg-primary-hover disabled:opacity-60"
          >
            {pending ? "Ingresando…" : "Ingresar"}
          </button>
        </form>

        <p className="mt-6 text-center text-[13px] text-text-faint">
          ¿No tenés cuenta?{" "}
          <Link href="/onboarding" className="font-semibold text-primary hover:text-primary-hover">
            Creá una
          </Link>
        </p>
      </div>
    </div>
  );
}
